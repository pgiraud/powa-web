...
function toggleTheme() {
  instance.value.proxy.$vuetify.theme.dark =
    !instance.value.proxy.$vuetify.theme.dark;
  localStorage.setItem(
    "dark_theme",
    instance.value.proxy.$vuetify.theme.dark.toString()
  );
}
...
const darkTheme = computed(() => {
  ...
  return instance.value.proxy.$vuetify.theme.dark;
});

function checkTheme() {
  const theme = localStorage.getItem("dark_theme");
  if (!_.isNull(theme)) {
    instance.value.proxy.$vuetify.theme.dark = Boolean(JSON.parse(theme));
  } else if (
    window.matchMedia &&
    window.matchMedia("(prefers-color-scheme: dark)").matches
  ) {
    instance.value.proxy.$vuetify.theme.dark = true;
    localStorage.setItem(
      "dark_theme",
      instance.value.proxy.$vuetify.theme.dark.toString()
    );
