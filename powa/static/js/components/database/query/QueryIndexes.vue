          ...
          <v-col cols="4">
            <ul v-if="data.indexes">
              <li v-for="(inds, clause) in data.indexes" :key="clause">
                <h5>
                  Possible indexes for attributes present in <b>{{ clause }}</b
                  >:
                </h5>
            ...
            <template v-if="data.hypoplan">
              <v-row>
                <v-col cols="12">
                  <template v-if="data.hypoplan.indexes">
                    ...
                    >:
                    <pre class="sql">
                      <code v-for="(ind, i) in data.hypoplan.indexes" :key="i">{{ind.ddl}}</code>
                    </pre>
                  </template>
                  <template v-else>
              ...
              <v-row>
                <v-col :cols="data.hypoplan.indexes ? 6 : 12">
                  ...
                  <pre
                    class="sql"
                  ><code data-equalizer-watch="">{{ data.hypoplan.baseplan }}</code></pre>
                </v-col>
                <v-col v-if="data.hypoplan.indexes" cols="6">
                  <pre>EXPLAIN plan <b>with</b> suggested index</pre>
                  <pre
                    class="sql"
                  ><code data-equalizer-watch="">{{ data.hypoplan.hypoplan }}</code></pre>
                </v-col>
                <v-col cols="12">
                  <pre>
